...
- **Dependencias**: Change 01
- **Notas críticas**:
  - `authStore` `partialize` persiste **solo `accessToken`**; el `usuario` se reconstruye con `GET /api/v1/auth/me` al rehidratar.
  - Estructura del `cartStore.items[*]`: `{ producto_id, nombre, precio, cantidad, imagen_url, personalizacion: number[] }`.
  - `paymentStore` y `uiStore` **sin persistencia**.
  - Sentar tokens base de Tailwind para alimentar el sistema de diseño que cierra el Change 27.

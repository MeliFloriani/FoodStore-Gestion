# backend-auth-login-deactivation Specification

## Purpose
Documents the deactivation invariants for the auth subsystem. Specifies how a deactivated user (soft-deleted by ADMIN via Change 21 admin-users-management) is blocked from login, from using existing Bearer tokens, and from renewing refresh tokens. All three mechanisms are implemented by existing Change 07 (auth-refresh-logout-rbac-me) code — no new code is required. Introduced as a capability documentation spec in Change 21 (admin-users-management).

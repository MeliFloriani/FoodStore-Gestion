# backend-orders-detail Specification

## Purpose
Provides the `GET /api/v1/pedidos/{id}` endpoint for retrieving the full detail of a single order, including items with snapshots, state history, user info, and associated payment. Introduced in Change 20 (orders-visualization).

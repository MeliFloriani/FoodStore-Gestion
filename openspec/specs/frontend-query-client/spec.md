# frontend-query-client Specification

## Purpose
Establish the TanStack Query 5 infrastructure for the Food Store frontend: a `QueryClientProvider` at app root with production-grade defaults (staleTime, gcTime, smart retry that skips 4xx, conditional refetchOnWindowFocus), and a centralized `queryKeys` factory with namespaced tuples for all domains. This layer is the single contract for server-state management — every feature query and mutation builds on top of it, ensuring cache coherence and consistent retry behavior across the application.

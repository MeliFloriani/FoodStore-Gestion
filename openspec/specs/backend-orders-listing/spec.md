# backend-orders-listing Specification

## Purpose
Provides the `GET /api/v1/pedidos` endpoint for paginated order listings with RBAC-based discrimination. CLIENT users see only their own orders; PEDIDOS and ADMIN roles see all system orders with optional filters. Introduced in Change 20 (orders-visualization).

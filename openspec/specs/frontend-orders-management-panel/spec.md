# frontend-orders-management-panel Specification

## Purpose
Provides the `PedidosPanelPage` at `/pedidos-panel` and `PedidosPanelDetailPage` at `/pedidos-panel/:id` for PEDIDOS/ADMIN staff to manage all system orders. Includes filters for state, date range, and client search with debounce. Also provides the `useAdminOrders` hook. Introduced in Change 20 (orders-visualization).

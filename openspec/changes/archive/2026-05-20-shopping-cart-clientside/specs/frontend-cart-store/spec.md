...
### Requirement: CartItem type — Breaking Change (producto_id and personalizacion)
`src/entities/cart/types.ts` SHALL update `CartItem` to use `string` for `producto_id` and `personalizacion`. This is a **Breaking Change** from the Change 05 spec (which used `number` and `number[]`), required to align with `ProductoPublicoRead.id: string` (Change 12) and `ProductoIngredienteRead.ingrediente_id: string` (Change 11).
...
The `AddItemResult` type is updated accordingly:
```typescript
...
### Requirement: updateQuantity action (from Change 05)
**Reason**: The `updateQuantity(producto_id, cantidad)` action addressed items by `producto_id` alone, which is insufficient when the same product has multiple personalization variants. It is replaced by the trio `incrementQuantity(itemKey)` / `decrementQuantity(itemKey)` / `setQuantity(itemKey, n)` which operate on `itemKey` and handle the `cantidad=0 → remove` edge case consistently.
**Migration**: Replace all `updateQuantity(id, n)` calls with `setQuantity(buildItemKey(item), n)`.

### Requirement: totalPrice selector (from Change 05)
**Reason**: `totalPrice` is replaced by `subtotal` (same computation, clearer semantics aligned with the checkout domain: `subtotal` + `costoEnvio` = `total`). `totalPrice` was a flat sum without room for the delivery cost component.
**Migration**: Replace `useCartStore(s => s.totalPrice())` with `useCartStore(s => s.subtotal())` or `useCartStore(s => s.total())` depending on intent.

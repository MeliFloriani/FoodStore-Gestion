...
`find_by_hash(token_hash: str) -> RefreshToken | None` SHALL also be added to `RefreshTokenRepository` (finds by hash without active check — used for replay detection).

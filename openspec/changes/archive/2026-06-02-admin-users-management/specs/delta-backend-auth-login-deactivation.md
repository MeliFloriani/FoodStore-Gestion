# DELTA: backend-auth-register-login (Deactivation Invariant)

**Change**: admin-users-management (Change 21)  
**Base spec**: `openspec/specs/backend-auth-register-login/spec.md`  

---

## ADDED Requirements

### Requirement: Login rejects deactivated users (deleted_at IS NOT NULL)

`AuthService.login_user` calls `UsuarioRepository.get_by_email` which applies a `WHERE deleted_at IS NULL` filter. A deactivated user (soft deleted by ADMIN) is therefore NOT found by `get_by_email`, causing the existing "not found" error path to execute.
...
### Requirement: get_current_user rejects Bearer tokens for deactivated users

`get_current_user` calls `uow.usuarios.get_by_id(user_uuid)` which applies `WHERE deleted_at IS NULL` via `BaseRepository.get_by_id`. A Bearer token for a deactivated user produces a `None` result, causing the existing "user not found" error path.

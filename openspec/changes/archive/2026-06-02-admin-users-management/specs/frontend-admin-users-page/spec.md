...
`src/features/admin-users/types.ts` SHALL define the following TypeScript interfaces with no `any` types and strict TypeScript compliance:
...
#### Scenario: UsuarioAdminRead has no password_hash field
- **WHEN** `UsuarioAdminRead` type is used to parse a backend response
- **THEN** the TypeScript interface has no `password_hash` field
- **THEN** accessing `user.password_hash` produces a TypeScript compile error

#### Scenario: UsersQueryParams is fully optional
- **WHEN** `useUsersQuery({})` is called with an empty object
- **THEN** no TypeScript error is raised (all fields are optional)
...
#### Scenario: Role filter triggers immediate callback
- **WHEN** user selects "ADMIN" in the role select
- **THEN** `onRoleChange("ADMIN")` is called immediately (no debounce)
- **THEN** parent component can pass `rol="ADMIN"` to `useUsersQuery`

#### Scenario: Status filter triggers immediate callback
- **WHEN** user selects "Activos" in the status select
- **THEN** `onActivoChange(true)` is called immediately
- **THEN** parent component can pass `activo=true` to `useUsersQuery`

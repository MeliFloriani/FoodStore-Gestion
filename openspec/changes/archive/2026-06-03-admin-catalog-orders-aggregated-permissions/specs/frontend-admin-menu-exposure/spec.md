...
The system SHALL guarantee that the menu rendered for a user with role ADMIN contains all management items. Specifically, `filterNavItems(NAVIGATION_ITEMS, ['ADMIN'])` SHALL return items covering all of the following paths: `/catalog`, `/cart`, `/orders`, `/profile`, `/addresses`, `/stock/products`, `/stock/categories`, `/stock/ingredients`, `/stock/inventory`, `/pedidos-panel`, `/admin/users`, `/admin/metrics`.

This invariant is guaranteed by the `allowedRoles` design in `NAVIGATION_ITEMS`: each management item includes `'ADMIN'` in its `allowedRoles` array. The `filterNavItems` function (Change 08) performs union filtering — any item with at least one role matching the user's roles is included.

#### Scenario: filterNavItems with ADMIN role includes all management items
...
- **THEN** the result includes ADMIN-only items with paths `/admin/users`, `/admin/metrics`
...
#### Scenario: Navigation component renders all management items for ADMIN user
...
- **THEN** the navigation renders links to `/admin/users` and `/admin/metrics`
...
The system SHALL provide a frontend test `navigation.admin.test.ts` that verifies the ADMIN menu exposure invariant. The test SHALL:
1. Call `filterNavItems(NAVIGATION_ITEMS, ['ADMIN'])` and assert all 12 expected items are present.
2. Render `<Navigation>` with mocked `useAuthStore` returning `status='authenticated'`, `user.roles=['ADMIN']` and assert all 12 expected nav links are rendered.
3. Assert no path is duplicated in the filtered result.

#### Scenario: navigation.admin.test.ts passes all assertions
